"""
Chapter 7 - SPX calibration
Section 06 - Short-term analysis

Short-term ATM skew: market vs Rough Heston vs classical Heston.

This script does not recalibrate either model. It reads the detailed short-term
calibration results and estimates the ATM skew consistently for the market,
Rough Heston and classical Heston using a local regression:

    sigma_impl(k,T) = a(T) + Skew(T) * k,
    k = log(K/F0(T)),

over the N_LOCAL observations closest to ATM-forward. When a calendar horizon
contains several effective maturities (SPX / SPXW), skews are aggregated using
a weighted average based on the number of local points.

No log-log regression or T^(H-1/2) power-law test is performed here; that is
reserved for the dedicated short-term asymptotic analysis.

Outputs:
- results/03_short_term_atm_skew_by_effective_maturity.csv
- results/04_short_term_atm_skew_by_calendar_horizon.csv
- figures/04_short_term_atm_skew_term_structure.png
"""
...
OUT_EFFECTIVE = (
    CSV_DIR
    / "03_short_term_atm_skew_by_effective_maturity.csv"
)
...
OUT_FIG = (
    FIGURES_DIR
    / "04_short_term_atm_skew_term_structure.png"
)
...
# ============================================
# 3. LOADING SHORT-TERM CALIBRATION RESULTS
# ============================================
...
if missing_rh:
    raise ValueError(
        "Missing columns in Rough Heston file: "
        + ", ".join(missing_rh)
    )

if missing_h:
    raise ValueError(
        "Missing columns in classical Heston file: "
        + ", ".join(missing_h)
    )
...
if he["Contract"].duplicated().any():
    raise ValueError(
        "The classical Heston file contains duplicate contracts."
    )
...
if not (merged["_merge"] == "both").all():
    ...
    raise ValueError(
        "The two calibrations do not contain exactly "
        "the same contracts.\n"
...
for col in numeric_common:
    ...
    if not np.allclose(
        ...
        raise ValueError(
            f"Inconsistency between files for column {col}."
        )
...
if actual_days != EXPECTED_DAYS:
    raise ValueError(
        f"Unexpected short-term horizons: {actual_days}. "
...
def local_atm_slope(
    ...
    The same set of strikes is used for all three series because local-point
    selection depends only on moneyness.
    """
...
for T, group in df.groupby(
    ...
    # ATM-point selection depends only on K/F0.
...
# slopes are aggregated using a weighted average based on
# the number of local points used, exactly as for the market
# skew constructed during data preparation.
...
# ===========================================
# 11. FIGURE: ATM SKEW AT SHORT MATURITIES
# ===========================================

fig, ax = plt.subplots(figsize=(9.5, 6.0))

short = skew_calendar.sort_values("Calendar_days").copy()
regular = short[short["Calendar_days"] >= 3].copy()
one_day = short[short["Calendar_days"] == 1].copy()
...
# The 1-day point is displayed separately.
if not one_day.empty:
    ...
    for column, marker in [
        ...
        y1 = float(one_day[column].iloc[0])
        if np.isfinite(y1):
            ax.scatter(
                [x1],
                [y1],
    ...
    market_1d = float(one_day["Skew_Market"].iloc[0])
...
ax.axhline(0.0, linewidth=0.9, linestyle=":", alpha=0.7)
ax.set_xticks([1, 3, 7, 15, 31])
...
print(
    ...
    "it is reserved for a dedicated asymptotic analysis."
)
