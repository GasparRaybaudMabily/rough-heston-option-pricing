"""
Calibration error analysis: Rough Heston vs classical Heston.

This script does not recalibrate either model. It reads the final detailed
global calibration results, verifies that Rough Heston and classical Heston
were evaluated on exactly the same contracts, and analyzes where their
calibration errors are concentrated.

The analysis covers errors by calendar horizon and forward moneyness region,
the average implied volatility bias by calendar horizon, and the relative RMSE
gain of Rough Heston compared with classical Heston.
"""
...
OUT_MATURITY = (
    CSV_DIR
    / "05_calibration_errors_by_maturity.csv"
)

OUT_MONEYNESS = (
    CSV_DIR
    / "06_calibration_errors_by_forward_moneyness.csv"
)

FIG_MATURITY = (
    FIGURES_DIR
    / "05_absolute_error_by_maturity.png"
)

FIG_MONEYNESS = (
    FIGURES_DIR
    / "06_absolute_error_by_forward_moneyness.png"
)

FIG_BIAS = (
    FIGURES_DIR
    / "07_bias_by_maturity.png"
)

FIG_GAIN = (
    FIGURES_DIR
    / "08_rough_heston_rmse_gain_by_maturity.png"
)
...
if df["Moneyness_zone"].isna().any():
    ...
    raise ValueError(
        "Some observations are outside the defined moneyness "
        f"regions. Min={outside.min():.6f}, "
...
ax.set_xlabel("Calendar horizon (days)")
ax.set_ylabel(
    r"Mean bias $\sigma_{\mathrm{model}}-\sigma_{\mathrm{market}}$"
)
...
ax.set_title(
    "Rough Heston calibration gain relative to classical Heston"
)
...
print(
    f"Rough Heston has the lower RMSE on "
    f"{n_rh_wins}/{len(errors_maturity)} horizons."
)

print(
    f"Classical Heston has the lower RMSE on "
    f"{n_h_wins}/{len(errors_maturity)} horizons."
)

print(
    f"Global Rough Heston RMSE gain: "
    f"{global_metrics['Gain_RMSE_RH_pct']:.2f} %"
)

print(
    f"Global Rough Heston MAE gain: "
